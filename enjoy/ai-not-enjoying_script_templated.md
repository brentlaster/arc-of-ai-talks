...
Let's start with the numbers. Research shows about 52% of workers are actively using AI tools. That's a lot. But here's the catch — worker confidence in using technology at work fell sharply by 18 percentage points over the same period, even as AI use rose. Adoption going up, confidence going down. More people using the tool, fewer people feeling good about it.
...
Now let me hit you with the uncomfortable numbers. Roughly three in four executives report difficulty scaling AI to repeatable ROI — that's from KPMG's 2026 CEO Outlook. Organizations are investing heavily, but most are still trying to figure out how to make this work beyond pilots and proofs of concept.
...
Meanwhile, Pew Research found that 52% of workers are more worried than hopeful about AI in their workplace. And organizations tracking productivity metrics — like ActivTrak — report that collaboration time increased notably while focused work sessions dropped after AI adoption. The tools multiply, but the deep-focus time shrinks.
...
First barrier. Survey data tells us developers are juggling more AI tools than ever — a 2025 survey found 59% of developers running three or more AI tools in parallel. IDE copilot, chat interfaces, code search, review assistants, doc generators. Each one is independently useful. But none of them share context with each other.

So YOU become the integration layer. You re-explain the same problem four times to four different systems. That human middleware cost is eating the productivity gains the tools are supposed to provide. The result? Widespread "shadow AI" use when the official tools are weak or slow.
...
Here's a consistent finding from the research: when teams standardize on two or three primary AI tools and use them deeply, they tend to get better results than teams spreading across six or seven tools used shallowly. BCG's data backs this up — perceived productivity rises going from one to two AI tools, peaks at three, and actually drops past four. It's not a universal law, but it holds up remarkably well.

Think of it like this: if you have one database, you understand your data. Three databases, you're managing replication. Five, you're debugging consistency issues. Same principle applies to AI tools. Past a certain point, the cost of managing the tools exceeds the benefit of having them.
...
Here's a quick example of what this looks like in practice: imagine AI generating PR summaries. They look great — concise, well-structured. But the summary describes a refactor as "minor cleanup" when the diff actually changes the retry logic for a payment processing path. The reviewer approves based on the summary. That's the "almost right" problem at the process level, not just the code level.
...
Let me walk you through a concrete example of the 'almost right' problem at the process level.
...
Fourth barrier, and this is the one that hits hardest. AI brain fry. For engineers, it's the constant loop of prompting, validating output line by line, re-prompting when it's wrong, and context-switching between tools — all while keeping your own mental model intact. A recent BCG and HBR study reported meaningful increases among heavy AI users: more mental effort, more fatigue, more information overload, and notably more decision fatigue. Their findings suggest roughly one in seven knowledge workers is experiencing significant cognitive strain from AI adoption. For engineering roles, the rates were even higher.

And here's the irony: AI was supposed to lighten workloads. But organizations tracking large user populations found that collaboration time increased notably while focused work sessions dropped after AI adoption. The workload didn't shift — it stacked. For engineers, that means more diff review fatigue, more re-prompt cycles, more context reconstruction between tools, and less uninterrupted time for the deep thinking that produces good architecture. You're debugging AI's understanding of your codebase instead of debugging the codebase itself.
...
Here's the data behind brain fry. BCG's research found that for organizations using AI heavily, collaboration time increased by an average of 14% while focused work dropped by 11% in the same period.

That sounds like small numbers. But for an engineer, 11% of focused time is 3-4 hours per week. That's your deep-thinking time. Your architecture time. Your 'understand the system better' time.
...
## SLIDE 21: AUDIENCE CHECK — WHICH BARRIER?

Quick check. Which of these five barriers is costing your team the most right now?
...
Cognitive overload tends to get the most recognition — BCG's survey found 14% of AI users reporting significant mental fatigue, and that number jumps to 26% in some departments. Here's a follow-up to think about: what percent of your team's AI time is generation versus verification? If your review overhead exceeds your generation time, you're not automating — you're just moving work. The shifts we're about to cover address that directly.
...
## SLIDE 23: SECTION DIVIDER — "FIVE ACHIEVABLE SHIFTS"
...
## SLIDE 25: EACH SHIFT ADDRESSES A BARRIER
...
## SLIDE 26: SHIFT 1 — ASSIGN AI THE RIGHT JOBS
...
## SLIDE 28: KNOW YOUR AI SWEET SPOTS
...
## SLIDE 29: SHIFT 2 — LIGHTER MENTAL MODELS
...
Or think of it another way: a fast collaborator with incomplete situational awareness. It sees the code in front of it but doesn't know your deployment topology, your on-call rotation, or what burned the team last sprint. Once you internalize that, you stop expecting oracle-level answers and start expecting useful first drafts. And that brings the checking cost back to something reasonable.

[TIMING: 31:45]

---

## SLIDE 30: TRUST SWEET SPOT
...
## SLIDE 32: CONTEXT MAKES THE DIFFERENCE
...
Same model. Same capability. A rule of thumb I keep coming back to: thirty extra seconds of context tends to save twenty minutes of re-prompting. That's the most efficient investment you'll make all day.

[TIMING: 35:45]

---

## SLIDE 33: SPOT THE MISSING CONTEXT
...
## SLIDE 34: SHIFT 4 — WORKFLOW PAIRING
...
Here's a quick example. Imagine asking AI to refactor a data access layer for performance. AI cleans up the queries beautifully — passes all unit tests. But in production, the refactored code hits the database with a new query pattern that bypasses connection pool limits and causes timeouts under load. The unit tests didn't cover production concurrency. That's a bolt-on failure: AI did the work, human skipped the integration review. When the workflow is redesigned so that AI refactors and a human explicitly checks production behavior — connection pooling, caching assumptions, query plans — the same task works reliably.

The key insight — and IDC's research on successful AI implementations supports this — is that organizations that redesigned workflows WITH AI got ROI. Organizations that just added AI to existing workflows didn't. The difference is integration versus bolt-on.

[TIMING: 39:30]

---

## SLIDE 35: CASE STUDY — AI CODE REVIEW

Here's a pattern that shows up consistently in engineering teams adopting AI review tools. A team starts using AI for code review. Initially it works well — AI catches formatting issues, obvious bugs, style inconsistencies. But then the team starts letting AI approve PRs on its own. Human reviewers start rubber-stamping. "AI already looked at it."
...
## SLIDE 37: TRAINING CHANGES EVERYTHING
...
## SLIDE 38: WHEN AI ACTUALLY SHINES
...
Code explanation and docs. Understanding unfamiliar codebases, generating docstrings, writing README sections, summarizing PR changes. AI excels at reading and summarizing code.
...
Here's a concrete example of what this looks like. Picture a legacy Java service with zero documentation. A new engineer joins and needs to understand the authentication flow. They feed the relevant classes to AI with the prompt: "Explain the authentication and authorization flow in this codebase. Identify the entry points, the middleware chain, and where tokens are validated." In 30 minutes, they have a working understanding that would have taken two days of reading code. That's AI earning its keep.
...
## SLIDE 39: LEGACY SYSTEM EXAMPLE — AI AS DOCUMENTATION

Here's what this looks like in practice. A new engineer joins a team with a legacy Java service. Ten thousand lines of code. Minimal docs. Confusing variable names. Scattered authentication logic.

Traditional approach: they spend two weeks reading code, asking questions, building a mental model. Slow, but thorough.

AI-powered approach: they take the core authentication classes, feed them to AI with context — "Explain the authn/authz flow. Show entry points, middleware chain, where tokens are validated" — and get a structured walkthrough in 30 minutes. Enough to be dangerous. Then they spend the next day diving deeper, knowing what to look for.

That's not replacing the engineer. It's giving them a head start. The engineer is still doing the thinking. AI is just doing the index-building faster.

[TIMING: 46:45]

---

## SLIDE 40: LEAD THE SHIFT — AUGMENTATION, NOT AUTOMATION
...
---
## SLIDE 41: YOUR SANE ADOPTION PLAYBOOK
...
## SLIDE 42: USING AI AND ENJOYING IT

Let me bring this home.

We started with a gap: 52% adoption, but confidence dropping. We mapped five barriers causing that gap. And we walked through five shifts that close it. Assign AI the right jobs. Adopt lighter mental models. Prompt with context first. Pair your workflows. Put guardrails in place.
...
| Barriers intro | 6-7 | 8:30 |
| Barrier 1: Tool sprawl | 8-10 | 12:45 |
| Barrier 2: Workflow fit | 11-12 | 16:30 |
| Barrier 3: Promise vs reality | 13-14 | 19:45 |
| Barrier 4: Brain fry | 15-16 | 22:45 |
| Midpoint interaction | 17 | 23:45 |
| Barrier 5: Pressure | 18 | 25:00 |
| Shifts intro | 19-20 | 26:00 |
| Shift 1: Right jobs | 21-23 | 30:15 |
| Shift 2: Mental models | 24-25 | 33:00 |
| Shift 3: Context-first | 26-28 | 37:45 |
| Shift 4: Workflow pairing | 29-30 | 41:00 |
| Shift 5: Guardrails + training | 31-32 | 43:45 |
| When AI shines | 33 | 45:45 |
| Leadership + playbook | 34-35 | 48:45 |
| Close + challenge | 36 | 50:00 |
| Q&A | 37 | ~59:00 |
| Ending | 38 | ~60:00 |

---

## WORD COUNT: ~7,250 spoken words | ~50 min content + ~10 min Q&A = 60 min
